...
def pairwise_similarity(features, aggregate=True):
    ...
    if aggregate:
        ...
        return sim[(1 - mask).bool()].flatten()
...
def inspect_in_class_sim(model, model_ckpts, single_loader, config, use_momentum=False, return_raw=False):
    ...
    for ckpt in tqdm(model_ckpts):
        ...
        for i in range(10):
            if not return_raw:
                ...
                class_dict_intra_q[i] = []
                class_dict_intra_k[i] = []
                class_dict_inter[i] = []
        ...
        with torch.no_grad():
            ...
            for img, label in single_loader:
                ...
                for i in range(10):
                    ...
                    if not return_raw:
                        ...
                        class_dict_intra_q[i].append(pairwise_similarity(z_masked, aggregate=False))
            ...
            for i in range(10):
                if not return_raw:
                    ...
                    class_dict_intra_q[i] = torch.cat(class_dict_intra_q[i])
    ...
    if not return_raw:
        ...
        return class_dict_intra_q_epoch

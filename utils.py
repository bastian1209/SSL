...
def adjust_temperature(epoch, config):
    t = config.train.temperature
    if epoch < config.train.temp_warmup:
        return t
    if config.train.temp_schedule == 'linear':
        t = t + (1.0 - t) * ((epoch - config.train.temp_warmup) / (config.train.epochs - 1 - config.train.temp_warmup))
    if config.train.temp_schedule == 'cos':
        t = t + 0.5 * (1. - math.cos(math.pi * (epoch - config.train.temp_warmup) / (config.train.epochs - 1 - config.train.temp_warmup))) * (1.0 - t)
...
def load_ssl_model(ckpt_path, method='moco'):
    ...
    elif (method == 'simclr') or (method == 'simsiam'):
...
def get_sl_model(model_path, arch, dataset_name, num_classes, method, eval_only=False, bn_encoder=True):
    encoder_params = {'BN' : bn_encoder, 'norm_layer' : None, 'is_cifar' : 'cifar' in dataset_name}
    model = ResNet_SL(arch_name=arch, encoder_params=encoder_params, num_classes=num_classes)

...
parser.add_argument('--eval-only', type=lambda x : x.lower() == 'true', default=False)
...
if __name__ == "__main__":
    ...
    config = get_config()
    config.defrost()
    parent_dir = os.path.dirname(args.model_path) if not args.eval_only else os.path.dirname(args.trained_path)
    config.merge_from_file(os.path.join(parent_dir, 'experiment_config.yaml'))
    config.train.base_lr = args.lr
    config.train.batch_size = args.batch_size
    config.system.save_period = args.save_period
    config.train.schedule = args.schedule
    config.train.milestones = [30, 40, 50] if (config.dataset.name == 'ImageNet_100' or 'cifar' in config.dataset.name) else config.train.milestones
